...
router.post('/v1/screener-question/answer', function (req, res) {
  ...
  if (answer == "yes"){
    ...
    res.redirect(`/v1/screener-question-live-work`);
  }
  else {
    // If the answer is no send to the no screen
    res.redirect(`/v1/screener-question-no`);
  }
...
router.post('/v1/screener-question-live-work/answer', function (req, res) {
  // Set a variable to the answer
  ...
  if (answer == "yes"){
    // If the answer is yes send to the next question
    res.redirect(`/v1/screener-question-leave-home`);
  }
  else {
    // If the answer is no send to the no screen
    res.redirect(`/v1/screener-question-no`);
...
// First screener page routing
router.post('/v2/screener-question/answer', function (req, res) {
  // Set a variable to the answer
  var answer = req.session.data['self-isolate']
  // Set prototype version
  var version = req.params[0];
  ...
  if (answer == "yes"){
    // If the answer is yes send to the next question
    res.redirect(`/v2/screener-question-leave-home`);
  }
  else {
    // If the answer is no send to the no screen
    res.redirect(`/v2/screener-question-no`);
  }
